...
class Elvwp_Dashboard_Widget {
	...
	public function elvwp_display_widget_contents() {
		...
		if ( isset( $_GET['elvwp-log-cleared'] ) && ! empty( $_GET['elvwp-log-cleared'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			printf( '<p><strong>%s</strong></p>', __( 'Log cleared.', 'error-log-viewer-wp' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		...
		?>
		<div class="elvwp-dashboard-widget-main">
			<a class="elvwp-view-log-link" href="<?php echo esc_url( admin_url( 'admin.php?page=error-log-viewer-wp' ) ); ?>"><?php esc_html_e( 'View Full Log', 'error-log-viewer-wp' ); ?></a>
