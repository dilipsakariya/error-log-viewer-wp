...
function elvwp_uninstall() {
	...
	$table_name = esc_sql( $wpdb->prefix . 'elvwp_error_logs' );
	$wpdb->query( "DROP TABLE IF EXISTS {$table_name}" ); // phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.SchemaChange
}
